...
async def cancelfunction(message:types.Message):
    ...
    if language =="uz": await message.answer("✅ Bosh menyuga xush kelibsiz!\n"
...
async def subcategory_products(message:types.Message,state:FSMContext):
    ...
    await state.update_data({ "level": "subcategory" })
    language = language_info(message.from_user.id)
    key = message.text[1:]
...
async def test(message:types.Message,state:FSMContext):
    ...
    if level=='subcategory':
        if language == 'uz':
            await message.answer("⬇ Kategoriyani tanlang", reply_markup=categories(language))
        else:
    ...
    if level=="category":
        ...
        await state.update_data({ "level": "category" })
        if language == 'uz':
            await message.answer("⬇ Kategoriyani tanlang", reply_markup=categories(language))
        else:
            await message.answer("⬇ Выберите категорию", reply_markup=categories(language))
...
async def test(message:types.Message,state:FSMContext):
    ...
    if 'subcategory' in category:
        await message.answer("⬇",reply_markup=product_or_subcategory(category=message.text,language=language))
    else:
        await state.update_data({ "level": "product-category" })
        ...
        if language == 'ru':
            ...
            await message.answer("⬇", reply_markup=button)
            await message.answer_photo(photo=data['image'],caption=f"<b>{data['name']}</b>\n\n{sena} : {data['price']} {money}",
                                       reply_markup=product_or_subcategory(category=message.text, language=language,product=data['id']))
...
async def decrease(call:types.CallbackQuery,callback_data:dict,state:FSMContext):
    ...
    if data['action'] =='add':
        ...
        if language == 'uz':
            ...
            await call.message.answer("⬇ Kategoriyani tanlang", reply_markup=categories(language))
        else:
            await call.message.answer('<i>Товар добавлен в вашу корзину.</i>')
            await call.message.answer("⬇ Выберите категорию", reply_markup=categories(language))